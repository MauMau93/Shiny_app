...
```{r}
ggplot(params$DATOS, aes(x=x, 
      fill = "Outcome")) + geom_histogram(position = "dodge") +
      labs(x = params$X_scatter,y = "Count") +
      scale_fill_manual(guide = F,values=c("#7A99AC", "#E4002B")) 

ggplot(params$DATOS, aes(x=y, fill = "Outcome")) + 
...
```{r}
ggplot(params$DATOS,aes(x=x,y=y,color = "Outcome"))+geom_point(size = 2) + 
...
clusters <- kmeans(variables_elegidas, params$clusters)
colors <- as.numeric(params$clusters)
scatterplot3d(variables_elegidas, color = colors,
